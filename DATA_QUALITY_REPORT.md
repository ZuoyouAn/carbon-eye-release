...
- `scripts/build_carbon_eye_data.py` 已重建：月度空气质量 152 条、长期气象 151 个完整月、园区快照 84 条、官方年度用电 4 条。
- 新增 2019-2025 `year_slots`：2020-2022 保持 `null`、`record_status="missing"` 和明确的缺失原因；不参与 EAI、CEI 和 CDCI。
- 购电间接排放复算遵循 `亿 kWh × 5.827 = 万吨 CO2`；2019、2023、2024、2025 均与 JSON 保留值一致。
- S08 长期气象来源已更新为“已下载并通过校验”；S01 原始空气质量平台、下载日期和许可保持待补状态。
- 新增 `data_provenance_registry.json`，含 12 项来源和关键文件 SHA-256 校验值。
